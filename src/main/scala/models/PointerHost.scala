...
case class PointerHost(
  ...
  def setName(newname: String) = PointerHost(cls, newname, ptrdname, timetolive)
  
  override def toAbsoluteNames(domain: ExtendedDomain) = 
    PointerHost(cls, HostnameUtils.absoluteHostName(name, domain.fullName), HostnameUtils.absoluteHostName(ptrdname, domain.fullName), timetolive)
  ...
  protected def getRData = new PTR((ptrdname.split("""\.""").map(_.getBytes) :+ Array[Byte]()).toList, timetolive)
